...
const adminDashboard = {
    ...
    async loadData() {
        try {
            const [studentResult, attResult, leaveResult, izinResult] = await Promise.all([
                api.getStudents(), api.getAllAttendance(), api.getAllLeaves(), api.getAllIzin()
            ]);
    ...
    updateStats() {
        ...
        const els = {
            'total-employees': totalEmployees, 'present-today': presentToday,
            'absent-today': absentToday, 'late-today': lateToday,
            'on-leave': onLeave, 'pending-requests': pendingLeaves + pendingIzin
        };
    ...
    campusOf(employee) { return employee?.kampus || employee?.campus || 'Kampus belum diisi'; },
    ...
    renderCampusAttendanceChart() {
        ...
        (this.employees || []).forEach(student => {
            ...
            if ((this.attendance || []).some(a => this.personMatches(student, a) && a.date === today && a.clockIn)) campusMap[campus].hadir++;
        });
        ...
        container.innerHTML = `<div style="padding:24px;width:100%;box-sizing:border-box;">${rows.map(([campus,value]) => {
            const percent = Math.round((value.hadir / max) * 100);
            return `<div style="margin-bottom:18px;"><div style="display:flex;justify-content:space-between;gap:12px;margin-bottom:7px;"><span style="font-weight:600;color:var(--text-primary);">${campus}</span><span style="font-weight:600;color:var(--text-secondary);">${value.hadir} hadir / ${value.total}</span></div><div style="height:10px;background:#eef2f7;border-radius:999px;overflow:hidden;"><div style="height:100%;width:${percent}%;background:#f59e0b;border-radius:999px;"></div></div></div>`;
        }).join('')}</div>`;
    },
    ...
    renderRecentActivity() {
        ...
        container.innerHTML = activities.map(act => `<div class="activity-item"><div class="activity-avatar"><img src="${getAvatarUrl(act)}" alt="${act.user}"></div><div class="activity-content"><p class="activity-text"><strong>${act.user}</strong> ${act.action}</p><span class="activity-time">${act.time}</span></div></div>`).join('');
    },
    ...
    renderOnlineUsers() {
        ...
        container.innerHTML = onlineUsers.map(user => `<div class="online-user-item"><div class="user-status-dot"></div><div class="activity-avatar"><img src="${getAvatarUrl(user)}" alt="${user.name}"></div><div class="activity-content"><p class="activity-text"><strong>${user.name}</strong></p><span class="activity-time">${this.campusOf(user)} - ${user.prodi || user.jurusan || user.position || '-'}</span></div></div>`).join('');
    },
...
// Load the academic/report compatibility layer on every SPA page.
if (!document.querySelector('script[data-academic-patch]')) {
    const s = document.createElement('script');
    s.src = 'js/academic-patch.js';
    s.dataset.academicPatch = '1';
